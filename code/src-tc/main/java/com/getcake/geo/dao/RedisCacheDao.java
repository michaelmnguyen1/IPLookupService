...
public class RedisCacheDao extends BaseDao {
	...
	public int loadCache (boolean flushCacheFlag, int lowerImportId, int upperImportId){
		...
		try {
			...
			jedis = new Jedis("localhost", hnp.getPort());
			System.out.println ("hnp.getHost(): localhost");
			jedis.connect();
			if (flushCacheFlag) {
				jedis.flushAll();
			}
...
"SELECT TOP 3 SUBSTRING(t.ipv6_start, 1, 3) as bytePrefixStr, SUBSTRING(t.ipv6_start, 4, 4) as byteStartStr, " +
	...
	public int loadIpLookupData (int maxNumOps) {
